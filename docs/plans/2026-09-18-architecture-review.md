...
- 2026-09-18：ponytail-review 发现 1–8 已执行并推送（`fbd60aa`，净 -44 行）：删除零读取状态字段、桌面 GPU 死链、未读的运行状态字段、重复的脉冲消息构造，合并双层校验。
- **发现 9 待用户决定**：`ResourceGovernor.Snapshot` / `ResourceSnapshot` / 接口成员仅测试读取——删（-9 行）或保留并在状态卡展示。
