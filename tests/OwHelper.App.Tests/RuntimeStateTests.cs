...
public class RuntimeStateTests
{
    ...
    static RuntimeState MakeState(FakeWindow window, Process self, int left, int top)
        => new RuntimeState(self.Id, self.StartTime.ToUniversalTime(), window.Handle.ToInt64(), left, top, false, 0);
    ...
    public void Store_SaveLoadClear_RoundTrips()
    {
        ...
        store.Save(new RuntimeState(42, DateTime.UtcNow, 0x5A81CL, 100, 200, false, 0));
