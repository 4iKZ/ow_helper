# OW 助手

**让《守望先锋 2》在后台自动按键的小工具。** 挂机时游戏不用占着屏幕——你可以切出去正常用电脑，想自己玩时点一下「停止挂机」即可。

## 它是干嘛的

- **解决什么问题**：战令、活动或训练场常常需要"每隔一会儿按一下键"。人得守在那儿，电脑也没法干别的。
- **它做什么**：发现游戏窗口后，在**后台**按你设定的节奏按键（鼠标左/右/中/侧键也支持）。游戏窗口可以不在前台，画面也不用停在屏幕最上层。
- **默认怎么跑**：点「开始挂机」就行——默认配置就是托比昂推荐值（每 30 秒自动按一次 Shift）。
- **它不做什么**：不注入游戏、不读写游戏内存、不装驱动、不做任何反作弊绕过——只发常规 Windows 窗口消息（详见[安全边界](#安全边界与平台条款)）。
- **会打扰我自己玩吗**：不会。默认在游戏窗口回到前台时自动跳过按键。

## 下载

在 [GitHub Releases](https://github.com/4iKZ/ow_helper/releases) 下载最新版：

- **推荐**：`OwHelper-Setup-<版本>.exe`（安装向导，下一步即可，自包含不用装 .NET）
- 绿色版：`OwHelper-win-x64-<版本>.zip`（自包含，解压即用）
- 高级用户：`OwHelper-win-x64-framework-dependent-<版本>.zip`（体积小，但要先装 [.NET 8 Desktop Runtime](https://dotnet.microsoft.com/download/dotnet/8.0)）

## 快速开始（四步）

1. **打开《守望先锋 2》**，进入你想挂的房间或训练场
2. 运行 **`OwHelper.Desktop.exe`**（刚下载的包里就有；自己构建见[给折腾党](#给折腾党技术细节)），会打开一个大窗口
3. 点**「开始挂机」**
4. 正常用电脑就行。想自己玩时，再点一次同一按钮停止；想立刻"眼不见心不烦"，点「老板键」

## 主窗口上都有什么

| 区域 | 说明 |
|---|---|
| 状态卡 | 连接状态 · 是否在挂机 · 已自动按键次数与下次倒计时 · 游戏窗口状态 · 最近一条提示 |
| 大按钮 | **开始挂机** / 运行中变成**停止挂机**（按钮下方会显示当前按键与间隔） |
| 老板键 | 一键让游戏"消失"：窗口移出屏幕 + 任务栏和 Alt+Tab 都看不到；再点「恢复游戏窗口」就回来 |
| 怎么用（四步） | 常驻的使用说明，不用记 |
| 更多设置… | 换按键（含鼠标左/右/中/侧键）、调按键节奏、后台省电模式等；「恢复推荐设置」可一键回到托比昂 Shift / 30 秒 |

## 关掉窗口会怎样？

**挂机不会中断**：程序会最小化到右下角托盘（首次会弹一个小提示）。想回来时**双击托盘图标**；想停止或退出，**右键托盘图标**即可。

托盘图标颜色就是状态：绿色=挂机中、黄色=等待/重连、红色=故障、灰色=已停止。

## 更多设置

- **自动按哪些键**：勾选 OW 常用键位（含鼠标左键/右键/中键/侧键、Shift、E、Q、Space、Ctrl、WASD 等）；也可以自己填英文键名
- **按键节奏**：每隔多少秒按一次、每次按住多久、间隔是否小幅随机
- **后台运行方式**：CPU 优先级别、后台省电模式（部分电脑不支持，会自动跳过并提示，不影响挂机）
- **其他**：玩游戏时自动暂停、允许隐藏游戏窗口、游戏重开后保持隐藏
- 保存后**立即生效**：正在挂机时，下一次自动按键就用新设置；改了间隔则立刻按新间隔重新计时

## 常见问题

| 现象 | 处理 |
|---|---|
| 状态卡显示"还没有找到《守望先锋》" | 先打开游戏；或右键托盘 → 重新连接游戏（程序每几秒会自动重试，也可以不管它） |
| 提示"后台省电模式没有生效" | 你的电源方案不支持，可忽略，不影响挂机 |
| 提示"按键发送失败了几次，可能是权限问题" | 用**管理员身份**运行 `OwHelper.Desktop.exe`（或把游戏改为普通权限运行） |
| 游戏窗口找不到了 | 主窗口点「恢复游戏窗口」；或右键托盘图标 → 恢复游戏窗口 |
| 点了老板键后游戏去哪了？ | 它还在正常运行，只是窗口被移到屏幕外并从任务栏/Alt+Tab 隐藏了；用「恢复游戏窗口」找回来 |
| 提示"OW 助手已在运行" | 本程序只能开一个：双击托盘图标打开已在运行的那个 |
| 会被封号吗？ | 工具只发常规窗口消息，不注入、不读写内存；但任何第三方工具都有风险，请阅读[安全边界与平台条款](#安全边界与平台条款)后自行判断 |

## 给折腾党（技术细节）

- **控制台版**（无窗口/脚本场景）：`src\OwHelper\bin\Release\net8.0-windows\OwHelper.exe [按键,逗号分隔] [间隔秒]`
- **诊断工具**：`BgKeyProbe.exe`（窗口清单、消息矩阵实验、单次脉冲测试）
- **配置文件**：`%LOCALAPPDATA%\OwHelper\config.json`（首次运行自动生成，界面保存时改写）
- **运行记录**：`%LOCALAPPDATA%\OwHelper\logs\owhelper-YYYYMMDD.log`（结构化事件，右键托盘 → 打开运行记录）
- **异常恢复**：窗口隐藏时异常退出会写 `runtime-state.json`，下次启动询问是否恢复
- **构建与测试**：`dotnet build ow_helper.sln -c Release` / `dotnet test ow_helper.sln -c Release`
  （测试包含使用替身窗口进程的实机集成测试与架构红线检查：无 P/Invoke 泄漏、无注入类 API）
- **发布**：`scripts\publish.ps1` → `artifacts\OwHelper-win-x64`（依赖 .NET 8 桌面运行时）
- **目录结构**：`src/OwHelper.Core`（Win32/输入/资源）、`src/OwHelper.App`（会话/配置/日志）、
  `src/OwHelper.Desktop`（桌面界面）、`src/OwHelper`（控制台）、`src/BgKeyProbe`（诊断）
- **协作与开发约定**：[AGENTS.md](AGENTS.md)；`docs/plans`、`docs/specs` 是历史设计记录，现役说明就是本文件

## 自动更新与完整性安全

本程序内置一键更新与校验能力（桌面端右键托盘或进入设置即可检查更新），更新设计遵循以下安全准则：

- **官方唯一信任锚**：程序仅认领 GitHub 官方 Releases 发布的资产信息与 `digest: sha256:...` 强校验摘要；
- **镜像源仅作传输通道**：内置的国内镜像加速仅用于加速二进制流下载，绝不作为信任锚点；下载后必须通过官方 SHA-256 验证；
- **校验失败绝对阻断**：任何哈希不匹配、文件大小异常或损坏包都会立刻销毁并拒绝执行，绝对不会拉起未知二进制文件；
- **下载过程挂机零打扰**：更新包在后台静默下载与校验，在此期间按键脉冲与老板键正常运行；只有在安装包完全就绪后才由用户确认停止；
- **安装过程安全闭环**：通过独立进程与脚本管理升级生命周期，仅在安装器成功完成（ExitCode 0）时拉起新版；安装器被取消或报错时绝不误拉起。

## Windows SmartScreen 提示说明

由于本工具为开源独立项目，尚未购买商业代码签名证书，在部分 Windows 系统首次下载运行安装包或可执行文件时，可能出现 Windows SmartScreen **「已保护你的电脑（未知发布者）」** 蓝色弹窗。

- 处理方式：点击弹窗中的 **「更多信息」**，然后点击 **「仍要运行」** 即可。
- 完整性核验：每个 GitHub Release 均随附官方 `SHA256SUMS.txt` 校验和文件，您可随时在 PowerShell 中执行 `Get-FileHash <文件路径> -Algorithm SHA256` 核对一致性。

## 支持环境

Windows 11 / Windows 10 x64 · .NET 8 Desktop Runtime · Overwatch 2（建议无边框窗口模式或窗口模式）

## 安全边界与平台条款

本工具不做（也永不做）：anti-ban、undetected mode、stealth、进程隐藏、内存注入、DLL 注入、内核驱动、游戏函数 Hook、输入驱动伪造、反作弊绕过。

请自行阅读并遵守：

- [Blizzard EULA](https://www.blizzard.com/en-us/legal/08b946df-660a-40e4-a072-1fbde65173b1/blizzard-end-user-license-agreement)
- [Blizzard Anti-Cheating Agreement](https://www.blizzard.com/en-us/legal/cd5930c0-2784-420c-a23d-1e0d6ff8599b/anti-cheating-agreement)

## License

MIT，见 [LICENSE](LICENSE)。
